...
class ScanSKU extends Component {
  ...
  showlogscan(logscans){
    ...
    if (logscans.length >0) {
      result = logscans.map((item, index) => {
        if(item.name == ''){
          return(
            <Text style={{fontWeight: 'bold',color:'green',borderTopWidth: 0.5,}}>
              {item.sku}
            </Text>
          );
        }
          ...
          return(
            <Text style={{fontWeight: 'bold',color:'#000000',borderTopWidth: 0.5,}}>
              {item.sku} - {item.name}
            </Text>
          );
        }
  ...
  savehistory(sku,name,status) {
    this.props.dispatch(savehistory(sku,name,status));
    this.setState({isShowConfirmBlock: false});
  }
  ...
  render() {
    ...
    return (
      <View style={styles.container}>
        <LogScan message={this.showlogscan(this.state.logscan)} title={'Lịch sử thao tác'} isShowAlertView={this.state.modalVisible} buttonTittle={'Đóng'} callBack={() => { this.closemodal() }}></LogScan>
        {camera}
        <AlertComfirmBlock isShowConfirmBlock={this.state.isShowConfirmBlock}
                            ...
                            callBack={() => { this.savehistory(this.state.sku,'',false) }}>
        </AlertComfirmBlock>
